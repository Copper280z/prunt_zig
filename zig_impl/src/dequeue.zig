...
pub fn Deque(comptime T: type) type {
    return struct {
        ...
        pub fn ensureTotalCapacity(deque: *Self, gpa: Allocator, new_capacity: usize) Allocator.Error!void {
            ...
            const target_capacity: usize = new_capacity +| (new_capacity / 2 + deque.buffer.len);
            return deque.ensureTotalCapacityPrecise(gpa, target_capacity);
        }
            ...
            if (gpa.remap(old_buffer, new_capacity)) |new_buffer| {
                ...
                if (deque.head > old_buffer.len - deque.len) {
                    ...
                    if (head.len > tail.len and new_buffer.len - old_buffer.len > tail.len) {
                        ...
                        // replace @memmove with std.mem.copyBackwards for 0.14 compatibility
                        std.mem.copyBackwards(T, new_buffer[deque.head..][0..head.len], head);
                    }
                }
